...
$data = "SELECT * FROM `blood_group` WHERE `blood_group_id` = '$record'";
        $query = mysqli_query($conn, $data);
        while($fetch = mysqli_fetch_assoc($query)){
            $bloodid = $fetch['blood_group_id']; 
            $bloodname = $fetch['blood_group_name']; 
            $blooddesc = $fetch['blood_group_desc']; 
                ...
                <main>
                    ...
                    <hr>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input class="form-control py-4" id="inputEmailAddress" type="text"  name="bloodgroupname" placeholder="" value="<?php echo $bloodname; ?>" />
                                            </div> 
                                           
                                            <div class="form-group">
                                                <label class="small mb-1" for="inputEmailAddress">Blood Group Description</label>
                                                 
                                                <textarea name="description" id=""class="form-control py-4" cols="30" rows="10"><?php echo $blooddesc ?></textarea>
                                            </div> 
                                             <button type="submit" class="btn btn-primary btn-block" name="uploadgroup">Update Blood Group</button>
                                             <?php
                                                        if(isset($_POST['uploadgroup'])){
                                                            include '../dbconnection.php';
                                                            $groupname = mysqli_real_escape_string($conn, $_POST['bloodgroupname']);
                                                        ...
                                                        $insert = "UPDATE `blood_group`  SET `blood_group_name` ='$groupname' ,`blood_group_desc`='$description' WHERE `blood_group_id`='$record'";
                                                                    $querys = mysqli_query($conn, $insert);
                                                                    if($querys){
                                                                        echo "<script>window.location.href='managebloodgroups.php';</script>";
                                                                    }
